...
instance Applicative D D D where
    df <*> da = D $ \dms ->
        let
            (f, dms')  = unD df dms
            (a, dms'') = unD da dms'
...
instance Return D where
  return a = D $ \dms -> (a, dms)
instance Fail D where
  fail = error
instance P.Applicative D where
  pure = pure
  (<*>) = (<*>)
...
instance Applicative m n p => Applicative (DFT m) (DFT n) (DFT p) where
    type ApplicativeCts (DFT m) (DFT n) (DFT p) = (ApplicativeCts m n p)
...
newtype DF a = DF (DFT D a) deriving (Functor)

instance Applicative DF DF DF where
    (DF mf) <*> (DF ma) = DF $ mf <*> ma
...
instance P.Applicative DF where
  pure = pure
  (<*>) = (<*>)
